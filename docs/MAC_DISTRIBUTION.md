...
> 当前 macOS 16 beta 环境中，Tauri 自动 `.dmg` 阶段使用的 `create-dmg` 可能失败。beta 包请优先使用 `scripts/build-macos-dmg.sh`。该脚本会先运行 `npm run build` 和 `cargo check`，再通过 Tauri 生成 `.app`，最后使用系统 `hdiutil` 创建包含 `AI 文档快看.app` 和 `Applications` 快捷方式的 DMG。

## 产物位置

| 产物 | 路径 |
|------|------|
| .app (Intel) | `src-tauri/target/release/bundle/macos/AI 文档快看.app` |
| .app (Apple Silicon) | `src-tauri/target/release/bundle/macos/AI 文档快看.app` |
| .dmg (Intel) | `src-tauri/target/release/bundle/dmg/AI 文档快看_1.9.0_x64.dmg` |
| .dmg (Apple Silicon) | `src-tauri/target/release/bundle/dmg/AI 文档快看_1.9.0_aarch64.dmg` |
| .dmg (Universal) | `src-tauri/target/release/bundle/dmg/AI 文档快看_1.9.0_universal.dmg` |
...
1. 构建后进入 bundle 目录：
   ```bash
   open "src-tauri/target/release/bundle/macos/AI 文档快看.app"
   ```
2. 验证应用启动正常，显示欢迎页
...
1. 双击打开 `.dmg` 文件
2. 将 `AI 文档快看.app` 拖入 `/Applications`
...
2. 右键 → **Get Info** → **Open with** → 选择 `AI 文档快看.app` → **Change All**
3. 双击 `.md` 文件，确认在 AI 文档快看中打开
...
```
"AI 文档快看.app" can't be opened because Apple cannot check it for malicious software.
```
...
```bash
xattr -d com.apple.quarantine "/path/to/AI 文档快看.app"
```
...
```bash
codesign -dvvv "src-tauri/target/release/bundle/macos/AI 文档快看.app"
spctl -a -v "src-tauri/target/release/bundle/macos/AI 文档快看.app"
```
...
codesign --deep --force --verify --verbose \
  ...
  "src-tauri/target/release/bundle/macos/AI 文档快看.app"

# 2. 创建 .dmg 后提交公证
xcrun notarytool submit "AI 文档快看_1.9.0_aarch64.dmg" \
  --keychain-profile "AC_PASSWORD" \
  --wait

# 3. 装订公证票据（staple）
xcrun stapler staple "AI 文档快看_1.9.0_aarch64.dmg"
```
